🚀 MassFlux-Gateway: C# to Java Migration Project
"레거시 C# 엔진을 Java 기반의 고성능 비동기 게이트웨이로 재설계한 마이그레이션 프로젝트입니다."

본 프로젝트는 기존 C# 기반 IoT 게이트웨이를 Java 17, Spring Boot, Netty 환경으로 이전하며 고성능 바이너리 데이터 처리와 확장성 있는 백엔드 구조를 확보하는 데 집중했습니다.
...
원인: 과거 자바 버전의 환경 변수 및 IDE 설정이 빌드 엔진과 충돌.

해결:

Maven Wrapper를 도입하여 프로젝트별 독립적인 빌드 환경을 구축.
...
해결: Netty의 Event-driven 아키텍처를 도입하여 논블로킹 방식으로 패킷을 수신하고, ByteBuf를 활용한 메모리 최적화 파싱 로직 구현.
...
# 1. 저장소 복제
git clone https://github.com/[사용자ID]/MassFlux-Gateway.git
...
📈 학습 및 성장 기록
C#과 Java의 차이 이해: BitConverter와 ByteOrder 간의 엔디안 처리 방식 차이를 극복하며 시스템 프로그래밍 이해도 증대.

백엔드 역량 강화: 백엔드 엔지니어로서의 역량을 키우기 위해 실무 수준의 트러블슈팅과 인프라(Docker) 최적화 과정을 기록함.
